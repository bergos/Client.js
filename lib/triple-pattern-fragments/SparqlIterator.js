...
function SparqlGroupIterator(source, group, options) {
  ...
  switch (group.type) {
  case 'bgp':
    var quads = group.triples.map(function (triple) {
      return _.extend(_.clone(triple), { graph: _.result(options, 'graph', null) });
    });
    return new ReorderingGraphPatternIterator(source, quads, options);
  case 'graph':
    childOptions = _.create(options, { graph: group.name });
