...
function RdfExtClient(store, options) {
  ...
  this._store = store;
  this._options = _.defaults(options, {defaultGraphIri: null});
}
...
RdfExtClient.prototype.getFragmentByPattern = function (pattern) {
  ...
  var graph     = rdf.isVariableOrBlank(pattern.graph)     ? this._options.defaultGraphIri : pattern.graph;
...
function StoreIterator(store, pattern) {
  ...
  store.match(pattern.graph, pattern.subject, pattern.predicate, pattern.object, function (graph) {
    graph = graph !== null ? graph.toArray() : [];

    self._matches = graph.map(function (triple) {
